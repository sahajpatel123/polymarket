...
mkdir -p "${CONFIG_DIR}/journal" "${CONFIG_DIR}/logs" "${CONFIG_DIR}/10h_report"
...
SESSION_JSON="${CONFIG_DIR}/10h_session.json"
python3 - <<PY
import json
...
Path("${SESSION_JSON}").write_text(json.dumps({
    ...
    "report_dir": "${CONFIG_DIR}/10h_report",
    "pid_file": "${CONFIG_DIR}/10h_paper.pid",
}, indent=2) + "\n")
print("session written", "${SESSION_JSON}")
PY

# Kill any existing paper process
...
echo "=== Starting 10h paper session at ${START_ISO} ==="
uv run polymaker run --paper --config-dir "${CONFIG_DIR}" \
  >> "${LOG_DIR}/stdout.log" 2>&1 &
PAPER_PID=$!
echo "${PAPER_PID}" > "${CONFIG_DIR}/10h_paper.pid"
...
pid = int("${PAPER_PID}")
log = Path("${CONFIG_DIR}/10h_heartbeat.log")
...
print("10h window complete; stopping paper if still running")
if alive(pid):
    try:
...
uv run python scripts/paper_12h_report.py --session "${CONFIG_DIR}/10h_session.json" --repo "${ROOT}" \
  | tee "${LOG_DIR}/10h_report_generation.log"

echo "=== 10h paper session finished ==="