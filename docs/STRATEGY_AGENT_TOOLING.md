...
2. `uv run python scripts/recovery_smoke.py --min-quotes 5529`
   ...
   unfrozen, requote runtime basis, paper-log family present, outage alerts
   cleared, and (when `logs/frozen_tape_snapshot.json` exists) quotes advanced
   past `quotes_at_freeze` (T1-106/T1-127). Use `--allow-stale-health` only for
   diagnose-during-DOWN.
