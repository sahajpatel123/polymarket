...
- **OOS holdout (last 30%):** `dn_quote=0` on a **thin** holdout (~4 baseline
  quotes) → **not replicated yet**. `validate_knob_candidate.py` flagged
  `oos_replicated=false`, `thin_holdout=true`.
- **Next:** re-validate after ≥24h paper / denser holdout; if OOS still fails,
  reject or combine with higher `trend_flow_z` instead.
