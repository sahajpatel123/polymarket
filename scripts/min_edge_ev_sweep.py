#!/usr/bin/env python3
"""Sweep min_edge_ticks for quote EV (OOS + significance).

Baseline: named profile min_edge_ticks (live_scaled ≈ 1).
Candidates: alternate floors. Tier-1 evidence only — does not change
defaults (T1-163). join_best_bid remains frozen separately.

Usage:
  uv run python scripts/min_edge_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--report", default="logs/min_edge_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = int(baseline.min_edge_ticks)
    values = [int(float(x.strip())) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"min_edge_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"min_edge_ticks": v})
            ...
            rows.append(row)
            print(
                f"min_edge={v} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        "slug": args.slug,
        "baseline_min_edge_ticks": base_v,
        "fill_mode": args.fill_mode,
        "values": values,
        ...
        "rows": rows,
        "note": (
            "changing default min_edge_ticks requires non-baseline finding=true "
            "with fills on multi-market tape; join_best_bid stays frozen"
        ),
    }
