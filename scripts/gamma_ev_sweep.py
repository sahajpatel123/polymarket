#!/usr/bin/env python3
"""Sweep gamma (inventory skew) for quote EV (OOS + significance).

Baseline: named profile gamma (live_scaled ≈ 0.6).
Candidates: alternate gamma. Documents whether skew strength moves EV
on the current tape (T1-157). Does not change defaults.

Usage:
  uv run python scripts/gamma_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--values", default="0.2,0.6,1.0,1.5")
    ...
    ap.add_argument("--report", default="logs/gamma_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_g = float(baseline.gamma)
    values = [float(x.strip()) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for g in values:
        with tempfile.TemporaryDirectory(prefix=f"gamma_ev_{g}_") as tmp:
            candidate = profile_from_overrides(baseline, {"gamma": g})
            ...
            row = {
                "gamma": g,
                "is_baseline": abs(g - base_g) < 1e-12,
            ...
            rows.append(row)
            print(
                f"gamma={g} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        ...
        "rows": rows,
        "note": (
            "changing default gamma requires non-baseline finding=true with fills "
            "on multi-market tape; inventory-skew Tier-2 still gated"
        ),
    }
