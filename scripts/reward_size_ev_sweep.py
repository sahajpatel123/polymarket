#!/usr/bin/env python3
"""Sweep reward_size_mult for quote EV (OOS + significance).

Baseline: named profile reward_size_mult (live_scaled ≈ 1.0).
Candidates: alternate reward-size floors. Tier-1 evidence only — does not
change defaults (T1-165). Relevant on zero-fill reward-path paper tape.

Usage:
  uv run python scripts/reward_size_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--values", default="1.0,1.5,2.0,2.5")
    ...
    ap.add_argument("--report", default="logs/reward_size_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = float(baseline.reward_size_mult)
    values = [float(x.strip()) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"reward_size_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"reward_size_mult": v})
            ...
            rows.append(row)
            print(
                f"reward_size_mult={v} finding={row['finding']} "
                f"ev_signal={row['ev_signal']} "
                f"n_quote={row['n_quote_baseline']}->{row['n_quote_candidate']} "
                f"holdout_dn_ev={row['holdout_ev_delta']} "
                f"reward_d={row['reward_accrual_delta']} p={row['paired_p']} "
                f"baseline={row['is_baseline']}"
            )

    report = {
        "slug": args.slug,
        "baseline_reward_size_mult": base_v,
        "fill_mode": args.fill_mode,
        "values": values,
        ...
        "rows": rows,
        "note": (
            "changing default reward_size_mult requires non-baseline finding=true "
            "with fills on multi-market tape; finding still needs n_fill>0"
        ),
    }
