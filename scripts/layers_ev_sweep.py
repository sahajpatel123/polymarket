#!/usr/bin/env python3
"""Sweep layers (quote depth) for quote EV (OOS + significance).

Baseline: named profile layers (live_scaled ≈ 3).
Candidates: alternate layer counts. Tier-1 evidence only — does not change
defaults (T1-159).

Usage:
  uv run python scripts/layers_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--report", default="logs/layers_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = int(baseline.layers)
    values = [int(float(x.strip())) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"layers_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"layers": v})
            ...
            rows.append(row)
            print(
                f"layers={v} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        ...
        "rows": rows,
        "note": (
            "changing default layers requires non-baseline finding=true with fills "
            "on multi-market tape"
        ),
    }
