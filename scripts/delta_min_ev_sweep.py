#!/usr/bin/env python3
"""Sweep delta_min_ticks (half-spread floor) for quote EV (OOS + significance).

Baseline: named profile delta_min_ticks (live_scaled ≈ 1).
Candidates: alternate floors. Tier-1 evidence only — does not change
defaults (T1-160).

Usage:
  uv run python scripts/delta_min_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--report", default="logs/delta_min_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = int(baseline.delta_min_ticks)
    values = [int(float(x.strip())) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"delta_min_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"delta_min_ticks": v})
            ...
            rows.append(row)
            print(
                f"delta_min={v} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        "slug": args.slug,
        "baseline_delta_min_ticks": base_v,
        "fill_mode": args.fill_mode,
        "values": values,
        ...
        "rows": rows,
        "note": (
            "changing default delta_min_ticks requires non-baseline finding=true "
