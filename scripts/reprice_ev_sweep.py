#!/usr/bin/env python3
"""Sweep reprice_ticks (quote churn threshold) for quote EV.

Baseline: named profile reprice_ticks (live_scaled ≈ 2).
Candidates: alternate thresholds. Tier-1 evidence for T2-05 churn work;
does not change defaults (T1-158).

Usage:
  uv run python scripts/reprice_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--values", default="1,2,4,8")
    ...
    ap.add_argument("--report", default="logs/reprice_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = int(baseline.reprice_ticks)
    values = [int(float(x.strip())) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"reprice_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"reprice_ticks": v})
            ...
            rows.append(row)
            print(
                f"reprice_ticks={v} finding={row['finding']} "
    ...
    report = {
        ...
        "rows": rows,
        "note": (
            "churn knob evidence for T2-05; changing default requires "
            "non-baseline finding=true with fills on multi-market tape"
        ),
    }
