#!/usr/bin/env python3
"""Sweep q_max_usdc (per-market inventory cap) for quote EV (OOS + significance).

Baseline: named profile q_max_usdc (live_scaled ≈ 250).
Candidates: alternate caps. Tier-1 evidence only — does not change
defaults (T1-164). Size/capital increases remain human-only per protocol.

Usage:
  uv run python scripts/q_max_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--values", default="50,125,250,500")
    ...
    ap.add_argument("--report", default="logs/q_max_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = float(baseline.q_max_usdc)
    values = [float(x.strip()) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"q_max_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"q_max_usdc": v})
            ...
            rows.append(row)
            print(
                f"q_max={v} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        ...
        "rows": rows,
        "note": (
            "changing default q_max_usdc requires non-baseline finding=true "
