#!/usr/bin/env python3
"""Sweep base_size_usdc (quote notional) for quote EV (OOS + significance).

Baseline: named profile base_size_usdc (live_scaled ≈ 50).
Candidates: alternate sizes. Tier-1 evidence only — does not change
defaults (T1-162). Size/capital increases remain human-only per protocol.

Usage:
  uv run python scripts/base_size_ev_sweep.py \\
...
def main() -> int:
    ...
    ap.add_argument("--values", default="5,25,50,100")
    ...
    ap.add_argument("--report", default="logs/base_size_ev_sweep/report.json")
    args = ap.parse_args()
    ...
    base_v = float(baseline.base_size_usdc)
    values = [float(x.strip()) for x in args.values.split(",") if x.strip()]
    rows: list[dict] = []

    for v in values:
        with tempfile.TemporaryDirectory(prefix=f"base_size_ev_{v}_") as tmp:
            candidate = profile_from_overrides(baseline, {"base_size_usdc": v})
            ...
            rows.append(row)
            print(
                f"base_size={v} finding={row['finding']} ev_signal={row['ev_signal']} "
    ...
    report = {
        "slug": args.slug,
        "baseline_base_size_usdc": base_v,
        "fill_mode": args.fill_mode,
        "values": values,
        ...
        "rows": rows,
        "note": (
            "changing default base_size_usdc requires non-baseline finding=true "
            "with fills on multi-market tape; size/capital increases are human-only"
        ),
    }
