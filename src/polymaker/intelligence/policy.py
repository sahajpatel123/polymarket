...
- One knob scales with bankroll, so a 10x capital change needs zero
  config edits.
- Daily-loss / drawdown / per-market caps stay proportional as the
  account compounds.
- Behavior is reviewable: a reader can sanity-check the risk envelope
  without doing arithmetic.
- It is the language the user asked for ("y% here, z% loss, w% growth").
...
# via ``config/policy.toml`` (Phase 1 stub) or environment variables.
# All values are fractions of capital.

DEFAULT_MAX_PER_MARKET_PCT = 0.05          # 5% per market, hard cap
...
DEFAULT_TARGET_DAILY_GROWTH_PCT = 0.10     # aspirational, logged not enforced
...
class RiskProfile:
    """Named risk posture multipliers. Multiply default percentages.

    The user picks a profile name once (``POLYMAKER_RISK_PROFILE``);
    we apply sensible multipliers on top of the base defaults.

    ``conservative``: 0.5x size caps, 0.5x daily loss kill, 0.5x target.
    ``balanced``:     1.0x everything (the factory defaults).
    ``aggressive``:   2.0x size caps, 2.0x daily loss kill, 2.0x target.
    """
    ...
    def from_name(cls, name: str) -> RiskProfile:
        ...
        if n == "conservative":
            return cls(name=n, size_mult=0.5, loss_kill_mult=0.5, target_mult=0.5, max_markets=4)
        if n == "aggressive":
            return cls(name=n, size_mult=2.0, loss_kill_mult=2.0, target_mult=2.0, max_markets=12)
        return cls(name=n, size_mult=1.0, loss_kill_mult=1.0, target_mult=1.0, max_markets=8)
...
class RiskPolicy:
    ...
    # Targets
    target_daily_growth_pct: float = DEFAULT_TARGET_DAILY_GROWTH_PCT
    ...
    def from_env(cls, profile_name: str | None = None) -> RiskPolicy:
        ...
        return cls(
            ...
            target_daily_growth_pct=cls._env_pct(
                "POLYMAKER_TARGET_DAILY_GROWTH_PCT", DEFAULT_TARGET_DAILY_GROWTH_PCT * prof.target_mult
            ),
    ...
    def resolve(self, capital_usdc: float) -> ResolvedPolicy:
        ...
        resolved policy; rejects negative values.
        """
        if capital_usdc < 0:
            raise ValueError(
        ...
        return ResolvedPolicy(
            ...
            target_daily_growth_usdc=capital_usdc * self.target_daily_growth_pct,
...
class ResolvedPolicy:
    ...
    target_daily_growth_usdc: float
