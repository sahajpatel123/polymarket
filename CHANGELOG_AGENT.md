...
2026-07-23T03:15:00Z | Tier1 | T1-114 minutes_to_critical whole-minute countdown; require while outage_open | pytest outage/validate; ~11.7h DOWN imminent=True; no Tier2 | merged
...
2026-07-23T04:05:00Z | Tier1 | T1-119 persist operator_mode/action in outage_status; validate requires keys | pytest outage/validate; ~12.5h DOWN critical; no Tier2 | merged
...
2026-07-23T12:30:00Z | Tier1 | Fill simulation for paper mode + backtester: paper mode now matches resting orders against live trade prints, tracks inventory/PnL/toxicity; replay backtester simulates fills; new scripts/backtest.py for offline PnL/reward/rebate analysis; multi-category scanner (--categories politics,sports,crypto,news); category field on MarketMeta + catalog store; docs/SELF_AUTOMATED_TRADER.md setup guide | pytest 259 passed, 2 skipped; ruff ok | merged
2026-07-23T13:10:00Z | Tier1 | Phase2 automation: scripts/pnl_tracker.py (daily PnL summary from metrics JSONL), scripts/market_health.py (connectivity + env + engine + paper + outage + PnL checks with webhook alerts), scripts/automated_trader.py (orchestrate paper/live with periodic health + backtest); 7 pnl_tracker tests | pytest 266 passed, 2 skipped; ruff ok on new scripts | merged
2026-07-23T13:40:00Z | Tier2 (PR) | Advanced quoting models: Avellaneda-Stoikov optimal pricing + Kelly-inspired sizing + risk-parity capital allocation. New pure-function modules in src/polymaker/strategy/ (avellaneda_stoikov.py, kelly.py, allocation.py, advanced_quoting.py). 42 new tests pass. Benchmark: advanced model 1.91x faster at p99 vs simple. Does NOT modify live engine path — opt-in via compute_advanced_quotes(). docs/ADVANCED_QUOTING.md with full math. Branch: strategy-acceleration. PR pending human review. | pytest 301 passed, 2 skipped; ruff ok; mypy pre-existing only | PR opened
2026-07-23T14:50:00Z | Tier1 | Auto market discovery + hot-reload: Engine.add_market/remove_market methods, _market_discovery_loop (periodic Gamma scan + score filter + auto-prune), _hot_reload_loop (watchfiles watches markets.toml for manual edits), MarketDataService.add_market/remove_market (dynamic WS subscribe), 6 new EngineConfig fields (auto_discovery_enabled, interval, tags, min_score, max_markets, profile, hot_reload). Off by default. 4 new tests. docs/AUTO_DISCOVERY.md. Fix: backtest now discovers all markets in multi-market journals. | pytest 305 passed, 2 skipped; ruff ok | merged
2026-07-25T13:29:56Z | Tier1 | Trade judgment: DecisionFramework on quote path (skip dead/stale, size/band from regime+learning, exits preserved); use_intelligence on live_scaled (config+livecfg); offline intel_decision_run | pytest 415 passed 2 skipped; tests/test_trading_judgment.py; implementer/intel_decision_run.txt ACCEPT | ready
2026-07-25T13:50:22Z | Tier1 | Fix judgment mapping: band_frac=0 is floor not no-op; adaptive toxic offsets always applied; map buy_offset+spread_mult; e2e toxic→strictly lower BUY | pytest 428 passed 2 skipped; test_toxic_fill_makes_quotes_more_defensive; SCRATCH intel_decision_run ACCEPT | ready
2026-07-25T14:21:29Z | Tier1 | P0 Trust: replay order lifecycle sync (cancel+partial live), shared build_targets pipeline (live+replay), EquityLedger, benchmark validity gates (no zero-trade PASS), capital feasibility, champion_challenger harness, intel ablations mode | pytest 450 passed 2 skipped; test_replay_lifecycle/validity/ledger/pipeline | partial roadmap
2026-07-25T14:51:00Z | Tier1 | Honest measurement: honest_pnl C/B/O rewards+AS haircut+undersize block; queue fill modes; 100k long-tape 0 divergence; degradation quarantine; intel ablations; financial claim gate rejects optimistic/monopoly; exit urgency | pytest 473+ passed; SCRATCH honest_measurement_run.txt ACCEPT | ready
...
2026-07-26T06:10:00Z | Tier1 | T1-149 opt-in join_best_bid (default off); join alone n_crossable=0; join+min_edge0 unlocks n_fill=33 optimistic but finding=false (degenerate CI) — keep frozen | pytest test_quoting; evidence/quant_edge/t1_149_join_best_bid.json | merged
2026-07-26T06:25:00Z | Tier1 | T1-150 fix bootstrap CI (LCG power-of-2 collapse); join+min_edge0 Newsom finding=true Vance=false; promo requires n_fill>0 — join still frozen | pytest; evidence/quant_edge/t1_150_bootstrap_ci_join_multimarket.json | merged
2026-07-26T06:40:00Z | Tier1 | T1-151 queue_ahead_sweep: join-BB optimistic fills are 100% equal-price; cons_ahead0=0 — conservative equal-price skip blocks promotion (not only queue=200) | pytest test_queue_*; evidence/quant_edge/t1_151_queue_ahead_equal_price.json | merged
2026-07-26T06:55:00Z | Tier1 | T1-152 through_price_tape: Newsom/Vance sells are 100% at-touch (n_through=0) — conservative join not viable on this tape; join reward in_band stays 1.0 | pytest test_through_price_tape; evidence/quant_edge/t1_152_through_price_tape.json | merged
